...
void assembleProblem(const Basis &basis,
  ...
  //auto isDirichlet = [] (auto x) {return (x[0]<1e-8 or x[0]>0.9999 or x[1]<1e-8 or x[1]>0.9999);};




  auto isDirichlet = [] (auto x) {return (x[0]<1e-8 or x[0]>0.9999);};
  ...
  //interpolate(*basis, rhs,DirichletValues, isDirichlet)
  ...
  for (size_t i=0; i<M.N(); i++){
    for (size_t j=0; j<M.N(); j++){
      if(j-1>=0 && i==j-1) M[i][j] = M[i][j];
      if(i-1>=0 && i-1==j) M[i][j] = M[i][j];
      if(i==j) M[i][j] = M[i][j];
    }
  }
