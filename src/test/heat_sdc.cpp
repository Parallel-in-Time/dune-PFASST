...
const size_t DIM = 1;            //Räumliche Dimension des Rechengebiets
...
namespace pfasst
{
  namespace examples
  {
    namespace heat_FE
    {
      ...
      void run_sdc(const size_t nelements, const size_t basisorder, const size_t dim, const size_t nnodes,
  ...
  int main(int argc, char** argv) {
    ...
    const size_t nelements = 10; //get_value<size_t>("num_elements", 1000); 
    const size_t nnodes    = 3; //get_value<size_t>("num_nodes", 3);
    const QuadratureType quad_type = QuadratureType::GaussRadau;
    const double t_0 = 0.0;
    const double dt = 0.25; //get_value<double>("dt", 0.025);
    double t_end = 0.5; //get_value<double>("tend", 0.1);
    size_t nsteps = 0; //get_value<size_t>("num_steps", 0);
    if (t_end == -1 && nsteps == 0) {
    ...
    const size_t niter = 10; //get_value<size_t>("num_iters", 10);
