...
namespace pfasst
{
  namespace examples
  {
	...
	//basis = FinEl->get_basis(nlevel);
        ...
        //this->dF_dune = Dune::DenseMatrix<Dune::FieldMatrix<double,NR_COMP,NR_COMP> >(this->A_dune.M(), this->A_dune.N());
...
//                   << "      |rel error| = " << LOG_FLOAT << encap::norm0(rel_error[num_nodes])
	...
	auto Mresult = this->get_encap_factory().create();
      ...
      Heat_FE<SweeperTrait, Enabled>::implicit_solve(shared_ptr<typename SweeperTrait::encap_t> f,
                                                    ...
                                                    const typename SweeperTrait::time_t& number_time_step,
      ...
      {                                                   
        ...
        auto fu = this->get_encap_factory().create();

        evaluate_f(fu, this->_old_time_states[(int) number_time_step-1][0] ,0,  fu);

        fu->data() *=-dt;
        rhs->data() += fu->data();
        rhs->data() += this->_old_time_states[(int) number_time_step-1][0]->data();
        
		
        Dune::BCRSMatrix<Dune::FieldMatrix<double,1,1> > M_dtA_dune = 	Dune::BCRSMatrix<Dune::FieldMatrix<double,1,1> >(this->A_dune);
        
        evaluate_df(M_dtA_dune, u, dt);
