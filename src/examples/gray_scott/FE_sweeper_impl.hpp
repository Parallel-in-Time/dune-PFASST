	...
	for(int i=0; i<u->data().size();++i)
	{
                                                       ...
                                                       const shared_ptr<typename SweeperTrait::encap_t> u)
      {
        ...
        //for (size_t i = 0; i < result->data().size(); i++) {
          ...
          //std::cout << "f->data im evaluate  " << u->data()[i] << std::endl;
        //}
        //std::exit(0);
                                                    ...
                                                    const shared_ptr<typename SweeperTrait::encap_t> rhs)
      {
        ...
        Dune::BCRSMatrix<Dune::FieldMatrix<double,SweeperTrait::NR_OF_COMP,SweeperTrait::NR_OF_COMP> > M_dtA_dune = 	Dune::BCRSMatrix<Dune::FieldMatrix<double,2,2> >(A_dune);
