...
namespace pfasst
{
  namespace examples
  {
	...
	typedef Dune::YaspGrid<2> GridType; 
        ...
        vtkWriter.write("gray_scott_end" + name);*/
...
//                   << "      |rel error| = " << LOG_FLOAT << encap::norm0(rel_error[num_nodes])
	...
	for (int i=0; i< 200 ;i++){
	  ...
	  Dune::CGSolver<VectorType> cg(linearOperator,
                              preconditioner,
                              1e-1, // desired residual reduction factor
	  ...
	  cg.apply(u->data(), newton_rhs , statistics ); //rhs ist nicht constant!!!!!!!!!
	    ...
	    //std::cout << "ergebnis u " << u->data()[i] << std::endl;
          ...
          evaluate_f(f, u, dt, rhs);
          this->_iterations += statistics.iterations;
          
          std::cout << i << " residuumsnorm von f(u) " << f->data().two_norm() << std::endl;  


	 auto tmp = f->data();
      		this->A_dune.mv(f->data(), tmp);
         double abbruch=  tmp*f->data() ;	

          //if(-abbruch <1e-10){          std::cout << "genauigkeit erreicht " << i << abbruch << std::endl; break; }
          if(f->data().two_norm() <1e-8){          std::cout << "genauigkeit erreicht " << i << std::endl; break; }
          
	  df.mv(u->data(), residuum->data());
          residuum->data() -= newton_rhs2;
          std::cout << "residuums norm " << residuum->data().two_norm() << std::endl;
