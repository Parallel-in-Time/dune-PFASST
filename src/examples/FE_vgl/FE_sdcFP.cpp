...
namespace pfasst
{
  namespace examples
  {
	...
	/*typedef Dune::YaspGrid<2> GridType; 
        GridType::LevelGridView gridView = FinEl->get_grid()->levelGridView(1);
        ...
        vtkWriter.addVertexData(sweeper->get_end_state()->data(), "fe_solution_u");

        vtkWriter.write("gray_scott_new");*/
	  ...
	  sweeper->get_end_state()->scaled_add(-1.0 , sweeper->exact(t_end));
    {
      ...
      std::cout << "Iterationen " << sweeper->_iterations << std::endl;
