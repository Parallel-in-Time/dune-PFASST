  ...
  namespace encap
  {
                                   ...
                                   const shared_ptr<Encapsulation<EncapsulationTrait>> y)
    {
      ...
      std::transform(this->get_data().begin(), this->get_data().end(), y->data().begin(),
                this->data().begin(),
                [a](typename EncapsulationTrait::vec_t  xi, typename EncapsulationTrait::vec_t   yi) { 
		  typename EncapsulationTrait::vec_t  pom(yi);
		  pom*=a;
		  pom+=xi;
		  return pom; });
...
//       std::transform(this->get_data().begin(), this->get_data().end(), y->data().begin(),
//                 this->data().begin(),
//                 [a](FieldVector<double,2> xi, FieldVector<double,2>    yi) { 
// 		  FieldVector<double,2> pom(yi);
// 		  pom*=a;
// 		  pom+=xi;
// 		  return pom; });
               ...
               >::type>::norm0() const
    {
      /*return std::abs(*(std::max_element(this->get_data().begin(), this->get_data().end(),
                               [](FieldVector<double,2> xi, FieldVector<double,2>  yi)
                                 { return xi.infinity_norm() < yi.infinity_norm(); }))); */
      return std::max_element(this->get_data().begin(), this->get_data().end(),
                               [](typename EncapsulationTrait::vec_t xi, typename EncapsulationTrait::vec_t yi)
                                 { return xi.infinity_norm() < yi.infinity_norm(); })->infinity_norm();
                              ...
                              const int tag, const bool blocking)
    {
      ...
      if (blocking) {
        ...
        comm->send(&(this->data()[0][0]), this->get_data().size()* this->get_data()[0].size(), dest_rank, tag);
      } else {
        //comm->isend(this->get_data().data(), this->get_data().size(), dest_rank, tag);
        comm->isend(&(this->data()[0][0]), this->get_data().size()* this->get_data()[0].size(), dest_rank, tag);
      }
    }
                              ...
                              const int tag, const bool blocking)
    {
      if (blocking) {
        ...
        comm->recv(&(this->data()[0][0]), this->get_data().size()* this->get_data()[0].size(), src_rank, tag);
      } else {
        //comm->irecv(this->data().data(), this->get_data().size(), src_rank, tag);
        comm->irecv(&(this->data()[0][0]), this->get_data().size()* this->get_data()[0].size(), src_rank, tag);
      }
               ...
               >::type>::bcast(shared_ptr<CommT> comm, const int root_rank)
    {
      ...
      comm->bcast(&(this->data()[0][0]), this->get_data().size()* this->get_data()[0].size(), root_rank);
    }
