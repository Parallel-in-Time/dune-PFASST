...
//                   << "      |rel error| = " << LOG_FLOAT << encap::norm0(rel_error[num_nodes])
	...
	/*for (int i=0; i<result->data().size(); ++i)
	{result->data()[i]= 8*this->_nu*this->_nu*u->data()[i]*u->data()[i]*(1.00-u->data()[i])/(this->_delta*this->_delta); 
	...
	//std::cout << "im impl_solve " << std::endl;
        for (size_t i = 0; i < u->get_data().size(); i++) {
	...
	for (int i=0; i< 20 ;i++){
	  ...
	  if(dirichletRightNodes[j])
          newton_rhs[j] = pow((1 + (pow(2, n/2.)-1 )* exp(-(n/2.)*d*(x+2*l1*t)) ), -2./n);

	  
	  x = -200;
	  if(dirichletLeftNodes[j])
	  newton_rhs[j] = pow((1 + (pow(2, n/2.)-1 )* exp(-(n/2.)*d*(x+2*l1*t)) ), -2./n);
	...
	//std::exit(0);
	//std::cout << "impl_solve_ende" << std::endl;
