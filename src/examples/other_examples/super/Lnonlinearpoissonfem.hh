...
class LNonlinearPoissonFEM :
  public Dune::PDELab::
    NumericalJacobianVolume<LNonlinearPoissonFEM<Param,FEM> >,
  public Dune::PDELab::
    NumericalJacobianApplyVolume<LNonlinearPoissonFEM<Param,FEM> >,
...
public:
  ...
  LNonlinearPoissonFEM (Param& param_, int incrementorder_=0)
    ...
    for (const auto& ip : rule)
      {
        ...
        //auto f=param.g(eg.entity(),ip.position());
        for (size_t i=0; i<lfsv.size(); i++)
          r.accumulate(lfsv,i, phihat[i]*factor);
      }
  }
                        ...
                        R& r) const
  {
    ...
    bool isdirichlet=1; //param.b(ig.intersection(),facecenterlocal);
    ...
    for (const auto& ip : rule)
      {
        ...
        auto j = 1; //param.j(ig.intersection(),ip.position());
        for (size_t i=0; i<lfsv.size(); i++)
          r.accumulate(lfsv,i,100*j*phihat[i]*factor);
      }
  }
    ...
    for (const auto& ip : rule)
      {
        ...
        auto q = u; //param.q(u);
        for (size_t i=0; i<lfsu.size(); i++){
          r.accumulate(lfsu,i,-(param.nu*gradu*gradphi[i][0])*factor);}
      }
  }
