...
namespace pfasst
{
  namespace examples
  {
	...
	if(rank==0) 
    for(int i=0; i< sweeper->get_end_state()->data().size(); i++) std::cout << "0 initial " << initial[i] << " naeherung " << naeherung[i] << " exact " << exact[i] << std::endl;
	MPI_Barrier(MPI_COMM_WORLD);  	
	if(rank==1) 
    for(int i=0; i< sweeper->get_end_state()->data().size(); i++) std::cout << "1 initial " << initial[i] << " naeherung " << naeherung[i] << " exact " << exact[i] << std::endl;
	MPI_Barrier(MPI_COMM_WORLD);  	
