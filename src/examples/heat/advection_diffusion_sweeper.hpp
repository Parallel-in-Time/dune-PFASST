  ...
  namespace examples
  {
    ...
    namespace advection_diffusion
    {
	  ...
	  std::shared_ptr<fe_manager> FinEl;
        ...
        public:
          //! @{
          explicit AdvectionDiffusionSweeper( std::shared_ptr<fe_manager> FinEl, size_t nvars)
          {
	    this->FinEl = FinEl;
	    ...
	    interpolate(*(FinEl->get_basis()), x_node, N_x);

	    interpolate(*(FinEl->get_basis()), q, exact_solution);
	    ...
	    FinEl->A_dune.mmv(u, rhs);
	    FinEl->A_dune.mmv(u, rhs2);
	    ...
	    Dune::MatrixAdapter<MatrixType,VectorType,VectorType> linearOperator(FinEl->M_dune);

	    Dune::SeqILU0<MatrixType,VectorType,VectorType> preconditioner(FinEl->M_dune,1.0);
	    ...
	    //encap::Dune_VectorEncapsulation<double,double> var(u.N());

	    //FinEl->M_dune.mv(f_impl, var);
	    //encap::Dune_VectorEncapsulation<double,double> neu(u.N());
	    ...
	    FinEl->M_dune.mv(rhs, M_rhs_dune);

	    Dune::BCRSMatrix<Dune::FieldMatrix<double,1,1> > M_dtA_dune = 	Dune::BCRSMatrix<Dune::FieldMatrix<double,1,1> >(FinEl->A_dune);
	    M_dtA_dune *= (dt * this->nu);
	    M_dtA_dune += FinEl->M_dune;
