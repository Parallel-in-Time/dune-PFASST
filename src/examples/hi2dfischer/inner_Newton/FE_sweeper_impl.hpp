...
namespace pfasst
{
  namespace examples
  {
	...
	FinEl->set_basis(_basis);
	std::cout << "basis erstellt" << std::endl;
	    
	assembleProblem(_basis, this->A_dune, this->M_dune);
	std::cout << "mass erstellt" << std::endl;
	...
	w = std::make_shared<VectorType>(this->M_dune.M());
        ...
        const auto bs = _basis->size();
        std::cout << "Finite Element basis of level " << nlevel << " consists of " <<  _basis->size() << " elements " << std::endl;
	...
	pom1.resize(_basis->size());
	pom2.resize(_basis->size());
        
	interpolate(*_basis, x_node, N_x);

	interpolate(*_basis, pom1, exact_solution1);
	interpolate(*_basis, result->data(), exact_solution1);
	...
	for (size_t i = 0; i < u->get_data().size(); i++) {
          //std::cout << i << " evaluate " << result->data()[i] << std::endl;
        }
	...
	for (int i=0; i< newton_max_steps ;i++){
	  ...
	  auto du = this->get_encap_factory().create();
	  ...
	  cg.apply(du->data(), newton_rhs , statistics ); //rhs ist nicht constant!!!!!!!!!
	  num_solves++;
          ...
          evaluate_f(f, u, dt, rhs);
          if(f->norm0()<this->newton){ if(!this->is_coarse) std::cout << "***************************************** anzahl iterationen innerer newton " << i+1 << " " << num_solves << f->norm0() <<std::endl;   break;}
