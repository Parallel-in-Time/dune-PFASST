...
if (!this->get_communicator()->is_first()) {
    ...
    if(this->get_communicator()->get_rank()==0){
	for(int i=0; i< this->get_fine()->initial_state()->data().size(); i++) std::cout << this->get_fine()->get_end_state()->data()[i]  << " new initial " << std::endl;
//std::exit(0);
	}*/	


    //if(this->get_communicator()->get_rank()==1) this->recv_coarse();
    //if(this->get_communicator()->get_rank()==0) this->send_coarse();
